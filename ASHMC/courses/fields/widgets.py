            ...
            for i, (option_value, option_label) in column:
                ...
                if has_id:
                    ...
                    final_attrs = dict(final_attrs, id='%s_%s' % (
                            attrs['id'], i))
                    label_for = u' for="%s"' % option_label#final_attrs['id']
                else:
                    label_for = ''
                    
                # this is not ideal, but useful for styling later.
                final_attrs['id'] = option_label
                ...
                output.append(u'<li><label class="colored %s"%s>%s %s</label></li>' % (
                        option_label, label_for, rendered_cb, option_label))
            output.append(u'</ul>')
        return mark_safe(u'\n'.join(output))
