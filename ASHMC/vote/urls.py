...
urlpatterns = patterns('vote.views',
       ...
       url('^create$', login_required(CreateMeasure.as_view()), name='vote_measure_create'),
       url('^create/ballot/(?P<num>\d+)$', login_required(GenBallotForm.as_view()), name="vote_gen_ballot_form"),
       url('^create/candidate/(?P<bnum>\d+)/(?P<cnum>\d+)$', login_required(GenCandidateForm.as_view()), name="vote_gen_ballot_form"),
